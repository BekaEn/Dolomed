...
test.describe('Extrakorporale Stoßwellentherapie page visual comparison', () => {
    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const url = lang === 'de' ? '/extrakorporale-stosswellentherapie/' : `/${lang}/extrakorporale-stosswellentherapie/`;
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ExtrakorporaleStosswellentherapiePage-Full-${lang}`,
                    'ExtrakorporaleStosswellentherapiePage',
                    test.info(),
                    {
                        threshold: 0.3,
                        includeAA: true,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ExtrakorporaleStosswellentherapiePage-Mobile-${lang}`,
                    'ExtrakorporaleStosswellentherapiePage',
                    test.info(),
                    {
                        threshold: 0.3,
