...
test.describe('Patienten & Zuweiser page visual comparison', () => {
    const languages = ['de', 'fr'] as const;
    
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ page }) => {
            // Set up the page with language-specific URL
            const url = lang === 'de' ? '/patienten-zuweiser/' : `/${lang}/patienten-zuweiser/`;
            ...
            const { diffCount, diffPath, actualPath } = await comparePageScreenshot(
                page,
                `PatientenZuweiserPage-Full-${lang}`,
                'PatientenZuweiserPage'
            );
        ...
        test(`Mobile view - ${lang}`, async ({ page }) => {
            ...
            const { diffCount, diffPath, actualPath } = await comparePageScreenshot(
                page,
                `PatientenZuweiserPage-Mobile-${lang}`,
                'PatientenZuweiserPage'
            );
