...
test.describe('Gefaesserkrankungen page visual comparison', () => {
    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const url = lang === 'de' ? '/gefaesserkrankungen/' : `/${lang}/gefaesserkrankungen/`;
                ...
                await page.waitForSelector('.elementor-element[data-id="627bb9d8"]', { 
                    state: 'visible',
                    timeout: 10000
                }).catch(error => {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `GefaesserkrankungenPage-Full-${lang}`,
                    'GefaesserkrankungenPage',
                    test.info(),
                    {
                        threshold: 0.3,
                        includeAA: true,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                try {
                    ...
                    await page.waitForSelector('.elementor-element[data-id="627bb9d8"]', { state: 'visible', timeout: 10000 });
                ...
                const result = await comparePageScreenshot(
                    page,
                    `GefaesserkrankungenPage-Mobile-${lang}`,
                    'GefaesserkrankungenPage',
                    test.info(),
                    {
                        threshold: 0.3,
