...
test.describe('Chronische Schmerzen page visual comparison', () => {
    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const url = lang === 'de' ? '/behandlung-von-chronischen/' : `/${lang}/behandlung-von-chronischen/`;
                ...
                await page.waitForSelector('.elementor-element[data-id="701a914"]', { 
                    state: 'visible',
                    timeout: 10000
                }).catch(error => {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ChronischeSchmerzenPage-Full-${lang}`,
                    'ChronischeSchmerzenPage',
                    test.info(),
                    {
                        threshold: 0.3,
                        includeAA: true,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                try {
                    ...
                    await page.waitForSelector('.elementor-element[data-id="701a914"]', { state: 'visible', timeout: 10000 });
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ChronischeSchmerzenPage-Mobile-${lang}`,
                    'ChronischeSchmerzenPage',
                    test.info(),
                    {
                        threshold: 0.3,
