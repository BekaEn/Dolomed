...
test.describe('Rückenschmerzen page visual comparison', () => {
    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const url = lang === 'de' ? '/behandlung-von-rueckenschmerzen/' : `/${lang}/behandlung-von-rueckenschmerzen/`;
                ...
                await page.waitForSelector('.elementor-element[data-id="3832e0ff"]', { 
                    state: 'visible',
                    timeout: 10000
                }).catch(error => {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `RueckenschmerzenPage-Full-${lang}`,
                    'RueckenschmerzenPage',
                    test.info(),
                    {
                        threshold: 0.3,
                        includeAA: true,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                try {
                    ...
                    await page.waitForSelector('.elementor-element[data-id="3832e0ff"]', { state: 'visible', timeout: 10000 });
                } catch (error) {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `RueckenschmerzenPage-Mobile-${lang}`,
                    'RueckenschmerzenPage',
                    test.info(),
                    {
                        threshold: 0.3,
